...
export const NewCardSchema = z
  .object({
    ...
    balance: z
      ...
      .regex(/^\d+(\.\d+)?$/, { message: "Balance must be a valid number" }),
  ...
  .superRefine((data, ctx) => {
    // اگر ارز ریال باشد و عدد حاوی نقطه یا اعشار باشد
    if (data.currency === "IRR" && data.balance.includes(".")) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Iranian Rial cannot contain decimals",
        path: ["balance"], // ارجاع خطا مستقیم به فیلد balance جهت نمایش در UI
      });
    }
  });
